...
export default function Navbar() {
  ...
  const handleLogout = async () => {
    const result = await Swal.fire({
      title: "Konfirmasi Logout", // Updated title
      text: "Anda yakin ingin keluar?", // Updated text
      icon: "warning",
      showCancelButton: true,
      confirmButtonText: "Ya, Keluar", // Updated button text
      cancelButtonText: "Batal", // Updated button text
      reverseButtons: true, // Optional: Puts confirm button on the right
    });
    ...
    if (result.isConfirmed) {
      try {
        ...
        navigate("/login");
        Swal.fire("Berhasil Keluar!", "Anda telah berhasil keluar.", "success"); // Updated messages
      } catch (error) {
        console.error("Error during logout:", error);
        Swal.fire(
          "Gagal Logout!",
          "Terjadi kesalahan saat mencoba keluar.",
          "error"
          ...
          <div className="flex flex-wrap justify-between items-center mx-auto max-w-screen-xl">
            ...
            <div className="flex items-center lg:order-2">
              {token && userInfo ? (
                <>
                  <Link
                    to={"/"} // Assuming this links to a user dashboard/profile
                ...
                <>
                  <Link
                    to={"/login"} // Corrected path to /login
                    className="text-gray-800 dark:text-white hover:bg-gray-50 focus:ring-4 focus:ring-gray-300 font-medium rounded-lg text-sm px-4 lg:px-5 py-2 lg:py-2.5 mr-2 dark:hover:bg-gray-700 focus:outline-none dark:focus:ring-gray-800"
                  >
                    Masuk
                  </Link>
                  <Link
                    to={"/register"} // Corrected path to /register
            ...
            >
              <ul className="flex flex-col mt-4 font-medium lg:flex-row lg:space-x-8 lg:mt-0">
                ...
                <li>
                  <Link
                    to={"/books"} // Assuming /books is the correct path
                ...
                {/* --- Added "Transaksi Saya" Menu Item --- */}
                {token && userInfo && (
                  <li>
                    <Link
                      to={"/my-transactions"}
